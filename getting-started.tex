...
In the example code, this is done at line 3, in the \texttt{setup} function, where the statement \inocodei{Serial.begin(9600)} sets the rate at \SI{9600}{baud}, that is, \num{9600} symbols per second.
...
Click on this menu and select ``9600 baud'' to set the receiver baud rate to the same value set in the transmitter, through the command \inocodei{Serial.begin(9600)} in the program \texttt{HelloWorld.ino}.
...
\subsection{Digital Input and Output}
Digital output has two states, high and low, which are often represented as~1 or~0.
A pin is low when its voltage is near ground and high when it is at or near the board supply voltage, which is \SI5V for the Arduino Uno Rev3, for example, or \SI{3.3}V for the ESP32.
Pins in an Arduino are identified by integers, which are labelled on the board.

To use a digital I/O pin, first it must be configured with the \texttt{pinMode} function, which takes as arguments the pin number and the mode identifier.
Arduino pins generally have 3 modes: \texttt{INPUT}, \texttt{OUTPUT}, and \texttt{INPUT\_PULLUP}.
To configure pin 5 as a digital output, for example, the \inocodei{pinMode(5, OUTPUT)}  function call is used.
...
To set pin 5 of an Arduino Uno to 5V, for example, the \inocodei{digitalWrite(5, 1)} or \inocodei{digitalWrite(5, HIGH)} function calls compile to the same exact instructions.
...
To get the state of pin 5 of an Arduino Uno, for example, the \inocodei{digitalRead(5)}  function call is used.
...
In the \texttt{loop} function, after each time the LED pin is toggled, the call to \inocodei{delay(1000)} blocks program execution for \SI{1000}{ms} before the next line of code is run.
...
In some situations, it is necessary to measure the time between events, not only stop for a given interval.
