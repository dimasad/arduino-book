...
In C and C++, this is done with the \texttt{if} and \texttt{if-else} statements.
...
The clause that follows the if can be a block statement.
Common errors: semicolon after the parenthesis.
Common errors: no parenthesis.
Else. 
If inside if.
Recommendation: avoid too nested structures.
