...
Below I list some common source of errors in students' programs related to operators:
\begin{description}
  \item[Equality] testing is done with double equal signs \texttt{==} while a single equal sign \texttt{=} is assignment to a variable.
  ...
  \item[Exponentiation,] the power operator $x^y$, is \emph{not} done with the hat operator \inocodei{x ^ y}, as in MATLAB.
  Instead, in C and C++, the hat sign \inocodei{x ^ y} performs the exclusive or (XOR) Boolean operation.
